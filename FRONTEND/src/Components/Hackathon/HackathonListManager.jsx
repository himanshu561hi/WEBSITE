...
export default function HackathonListManager({ onSelectHackathon = null }) {
  ...
  const handleDeleteHackathon = async (hackathon) => {
    if (hackathon.status === "ACTIVE") {
      toast.error("Cannot delete an ACTIVE hackathon. Please mark it as Draft or Completed first.");
      return;
    }

    const confirmed = window.confirm(
      `Are you sure you want to delete "${hackathon.name}" (${hackathon.hackathonId})?\n\nThis will permanently delete this hackathon. This action cannot be undone.`
    );
    if (!confirmed) return;
    ...
    try {
      setActionLoading(true);
      const token = getAdminToken();
      const res = await axios.delete(`${BACKEND_URL}/api/hackathons/${hackathon.hackathonId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });

      if (res.data?.success) {
        toast.success(res.data.message || `Hackathon "${hackathon.name}" deleted.`);
        const storedId = localStorage.getItem("adminSelectedHackathonId");
        if (storedId === hackathon.hackathonId) {
                  ...
                  <div className="flex items-center gap-2 flex-wrap self-end lg:self-center">
                    ...
                    {h.status !== "ACTIVE" ? (
                      <button
                        ...
                        disabled={actionLoading}
                        className="px-2.5 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 text-xs font-bold rounded-xl transition flex items-center gap-1 cursor-pointer border border-rose-200 disabled:opacity-50"
                      ...
                      <button
                        disabled
                        className="px-2.5 py-1.5 bg-slate-100 text-slate-400 text-xs font-bold rounded-xl flex items-center gap-1 cursor-not-allowed opacity-60"
                        title="Active hackathon cannot be deleted. Deactivate or switch active first."
                      >
              ...
              <div className="pt-4 border-t border-slate-100 flex items-center justify-between gap-2">
                {selectedHackathon?.status !== "ACTIVE" ? (
                  <button
                    ...
                    onClick={() => {
                      setShowEditModal(false);
                      handleDeleteHackathon(selectedHackathon);
                    }}
                    className="px-3.5 py-2 bg-rose-50 hover:bg-rose-100 text-rose-700 text-xs font-bold rounded-xl transition flex items-center gap-1.5 border border-rose-200 cursor-pointer"
                  >
                    <Trash2 className="w-3.5 h-3.5 text-rose-600" /> Delete Hackathon
                  </button>
                ) : (
                  <div />
                )}
